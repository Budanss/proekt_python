...
def positive_assert(first_name):
    user_body = get_user_body(first_name) # Сохраняем обновлённое тело запроса
    user_response = sender_stand_request_post.post_new_user(user_body)# Сохраняем результат запроса на создание пользователя
    assert user_response.status_code == 201
    assert user_response.json()["authToken"] != ""

    users_table_response = sender_stand_request_get.get_users_table()  # Сохраняется результат запроса на получение данных из таблицы user_model
...
def test_create_user_no_first_name_get_error_response():
    user_body = data.user_body.copy()    # Копируется словарь с телом запроса
